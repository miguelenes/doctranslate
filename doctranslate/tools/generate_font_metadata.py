...
def get_font_metadata(font_path) -> PdfFont:
    ...
    translation_config.progress_monitor = (
        doctranslate.format.pdf.high_level.ProgressMonitor(
            doctranslate.format.pdf.high_level.get_translation_stage(translation_config)
        )
    )
    translation_config.font = font_path
    il_creater = doctranslate.format.pdf.high_level.ILCreater(translation_config)
    il_creater.mupdf = doc
    buffer = io.BytesIO()
    doc.save(buffer)
    doctranslate.format.pdf.high_level.start_parse_il(
        buffer,
        doc_zh=doc,
        resfont="test_font",
