...
from doctranslate.format.pdf.document_il.midend.add_debug_information import (
    AddDebugInformation,
)
...
from doctranslate.format.pdf.document_il.xml_converter import XMLConverter
from doctranslate.format.pdf.pdfinterp import PDFPageInterpreterEx
from doctranslate.format.pdf.result_merger import ResultMerger
...
from doctranslate.utils import memory
...
# PDF producer metadata from this fork (DocTranslater) or earlier branding (DocTranslate).
_PRIOR_TRANSLATION_PRODUCER_MARKERS = ("DocTranslater", "DocTranslate")


def _producer_marks_prior_translation(producer: str) -> bool:
    if "Translation_generated_by_AI,please_carefully_discern" not in producer:
        return False
    return any(m in producer for m in _PRIOR_TRANSLATION_PRODUCER_MARKERS)
...
def add_metadata(
    ...
    for attr in (
        ...
        translated_by = (
            f"DocTranslater{WATERMARK_VERSION}_{time.time()}_"
            "Translation_generated_by_AI,please_carefully_discern"
        )
