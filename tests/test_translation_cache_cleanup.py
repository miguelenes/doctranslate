...
def test_cleanup_under_limit(monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr("doctranslate.translator.cache.CLEAN_PROBABILITY", 1.0)
        # Lower the MAX_CACHE_ROWS threshold for quick test execution
        monkeypatch.setattr("doctranslate.translator.cache.MAX_CACHE_ROWS", 1000)
...
def test_cleanup_over_limit(monkeypatch):
    ...
    try:
        cache = TranslationCache("dummy")
        monkeypatch.setattr("doctranslate.translator.cache.CLEAN_PROBABILITY", 1.0)
        monkeypatch.setattr("doctranslate.translator.cache.MAX_CACHE_ROWS", 500)
